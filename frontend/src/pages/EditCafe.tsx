...
export default function EditCafe() {
  ...
  const [latitude, setLatitude] = useState(state.cafe.latitude.toString());
  const [longitude, setLongitude] = useState(state.cafe.longitude.toString());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await cafeService.update(state.cafe.id, {
        name,
      ...
      setError(
        err.response?.data?.error ||
          "Failed to update cafe. Make sure User #2 exists!",
      );
    } finally {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white border rounded-lg shadow-sm font-sans">
      <h1 className="text-2xl font-bold mb-6">Edit Study Spot</h1>
